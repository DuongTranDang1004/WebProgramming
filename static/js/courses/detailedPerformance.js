    ...
    fetch(`/api/lectures/course/${courseId}`)
      ...
      .then((data) => {
        ...
        fetch(`/api/boughtCourses/${boughtCourseId}`)
          ...
          .catch((error) =>
            console.error("Error fetching bought course details:", error)
          );
      })
    ...
    fetch(`/api/lectures/${lectureId}`)
      ...
      .then((data) => {
        ...
        fetch(`/api/boughtCourses/${boughtCourseId}`)
          .then((response) => response.json())
          .then((boughtCourse) => {
            const lectureStatus = boughtCourse.completedLectures.find(
              (status) => status.lectureId === lectureId
            );

            const exerciseForm = document.getElementById("exerciseForm");
            const submitButton = exerciseForm.querySelector(
              'button[type="submit"]'
            );
            ...
            if (lectureStatus && lectureStatus.completeStatus) {
              ...
              const userAnswer = boughtCourse.userAnswers.find(
                (answer) => answer.lectureId === lectureId
              );
              if (userAnswer) {
                resultElement.textContent = `Your previous answer: ${
                  userAnswer.answer
