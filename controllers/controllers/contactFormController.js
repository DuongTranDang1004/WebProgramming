 ...
 *         _id:
 *           type: string
 *           description: The auto-generated ID of the contact form (generated by MongoDB)
 *           example: 60f73b9f2f4a2b4b0c7df712
 *         contactPurpose:
 *           type: string
...
const createContactForm = async (req, res) => {
  try {
    ...
    res.status(400).json({
      message: "An error occurred while creating the contact form",
      error,
    });
  }
};
...
const updateContactForm = async (req, res) => {
  try {
    const id = req.params.id;
    const updatedForm = await ContactForms.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    if (!updatedForm) {
      return res.status(404).json({ message: "Contact form not found" });
